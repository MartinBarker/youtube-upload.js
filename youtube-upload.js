...
console.log('begin')
main('title1', 'desc2', 'tags1')

function main(title, description, tags) {
...
function authorize(credentials, callback) {
    ...
    var dontStoreToken = true

    // Check if we have previously stored a token.
    fs.readFile(TOKEN_PATH, function (err, token) {
        if (err) {
            console.log('TOKEN_PATH not found')
            getNewToken(oauth2Client, callback);
        } else {
            console.log('TOKEN_PATH found')
            if(dontStoreToken){
                console.log('dontStoreToken=True so treat as new run')
                getNewToken(oauth2Client, callback);
            }else{
